...
const Basket = () => {
  ...
  const totalPrice = listOfIds.reduce((acc, rec) => {
    const productPrice = (goodsInBasket.find((item) => item.id === rec.id)?.price || 0) * rec.quantity
            ...
            <div className="flex-1 shadow-lg pin-r pin-y p-8">
              <table className="w-full text-sm lg:text-base" cellSpacing="0">
                ...
                {goodsInBasket.map((item) => {
                  const { quantity } = listOfIds.find((it) => it.id === item.id)
                  return (
                    <tbody key={item.id}>
                      <tr>
                        <td className="hidden pb-4 md:table-cell">
                          <img
                            alt={item.title}
                            className="product__image block object-cover h-24 w-24 rounded-full"
                            src="https://picsum.photos/600/400/?random"
                          />
                        </td>
                        <td className="product__title font-semibold mb-2 md:ml-4">{item.title}</td>
                        ...
                        <td className="hidden text-right md:table-cell">
                          ...
                          <span className="text-sm lg:text-base font-medium">{symbol}</span>
                        </td>
